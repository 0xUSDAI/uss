...
contract JugTest is DSTest {
    ...
    function draw(bytes32 ilk, uint usdai) internal {
        vat.file("Line", vat.Line() + rad(usdai));
        vat.file(ilk, "line", line(ilk) + rad(usdai));
    ...
    function test_drip_0d() public {
        jug.init("i");
        jug.file("i", "duty", 1000000564701133626865910626);  // 5% / day
        assertEq(vat.usdai(ali), rad(0 ether));
        jug.drip("i");
        assertEq(vat.usdai(ali), rad(0 ether));
    }
    function test_drip_1d() public {
        ...
        jug.file("i", "duty", 1000000564701133626865910626);  // 5% / day
        hevm.warp(now + 1 days);
        assertEq(wad(vat.usdai(ali)), 0 ether);
        jug.drip("i");
        assertEq(wad(vat.usdai(ali)), 5 ether);
    }
    function test_drip_2d() public {
        ...
        assertEq(wad(vat.usdai(ali)), 0 ether);
        jug.drip("i");
        assertEq(wad(vat.usdai(ali)), 10.25 ether);
    }
    function test_drip_3d() public {
        ...
        assertEq(wad(vat.usdai(ali)), 0 ether);
        jug.drip("i");
        assertEq(wad(vat.usdai(ali)), 15.7625 ether);
    }
    function test_drip_negative_3d() public {
        ...
        hevm.warp(now + 3 days);
        assertEq(wad(vat.usdai(address(this))), 100 ether);
        vat.move(address(this), ali, rad(100 ether));
        assertEq(wad(vat.usdai(ali)), 100 ether);
        jug.drip("i");
        assertEq(wad(vat.usdai(ali)), 92.6859375 ether);
    }
    ...
    function test_drip_multi() public {
        ...
        hevm.warp(now + 1 days);
        jug.drip("i");
        assertEq(wad(vat.usdai(ali)),  15.5 ether);
    ...
    function test_drip_base() public {
        ...
        hevm.warp(now + 1);
        jug.drip("i");
        assertEq(wad(vat.usdai(ali)), 10 ether);
    }
