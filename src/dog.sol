...
/// dog.sol -- USDai liquidation module 2.0
...
contract Dog {
    ...
    struct Ilk {
        ...
        uint256 hole;  // Max USDAI needed to cover debt+fees of active auctions per ilk [rad]
        uint256 dirt;  // Amt USDAI needed to cover debt+fees of active auctions per ilk [rad]
    }
    ...
    uint256 public Hole;  // Max USDAI needed to cover debt+fees of active auctions [rad]
    uint256 public Dirt;  // Amt USDAI needed to cover debt+fees of active auctions [rad]
    ...
    //
    // Liquidate a Vault and start a Dutch auction to sell its collateral for USDAI.
    //
    // The third argument is the address that will receive the liquidation reward, if any.
    //
    // The entire Vault will be liquidated except when the target amount of USDAI to be raised in
    ...
    // outstanding USDAI target. The one exception is if the resulting auction would likely
        ...
        uint256 dust;
        {
            ...
            if (art > dart) {
                if (mul(art - dart, rate) < dust) {
                    ...
                    // the extra amount of target USDAI over the limits intended is not of economic concern.
                    dart = art;
                } else {
