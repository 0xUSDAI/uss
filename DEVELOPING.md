# Multi Collateral USDai Developer Guide
...
USDai has three different numerical units: `wad`, `ray` and `rad`
...
The core CDP, USDai, and collateral state is kept in the `Vat`. This
contract has no external dependencies and maintains the central
"Accounting Invariants" of USDai.

USDai cannot exist without collateral:
...
CDP of user `u`, using `gem` from user `v` and creating `usdai` for user
`w`.
...
equal quantity of USDai using `heal(u, v, rad)`: take `sin` from `u` and
`usdai` from `v`.

Note that `heal` can also be used to *create* USDai, balanced by an equal
quantity of Sin.

Finally, the quantity `usdai` can be transferred between users with `move`.
...
which increases the `usdai` balance of the user `u` by increasing the
...
- `spot`: the maximum amount of USDai drawn per unit collateral
- `line`: the maximum total USDai drawn

And a global risk parameter:

- `Line`: the maximum total USDai drawn across all ilks
