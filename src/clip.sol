...
/// clip.sol -- USDai auction module 2.0
...
contract Clipper {
    ...
    address     public vow;      // Recipient of usdai raised in auctions
    ...
    struct Sale {
        ...
        uint256 tab;  // USDai to raise       [rad]
    ...
    // 
    // Auctions will not collect more USDAI than their assigned USDAI target,`tab`;
    // thus, if `amt` would cost more USDAI than `tab` at the current price, the
    // amount of collateral purchased will instead be just enough to collect `tab` USDAI.
    //
    // To avoid partial purchases resulting in very small leftover auctions that will
    // never be cleared, any partial purchase must leave at least `Clipper.chost`
    // remaining USDAI target. `chost` is an asynchronously updated value equal to
    ...
    function take(
        ...
        uint256 max,          // Maximum acceptable price (USDAI / collateral) [ray]
...

        {
            ...
            // USDAI needed to buy a slice of this sale
            owe = mul(slice, price);

            // Don't collect more than tab of USDAI
            if (owe > tab) {
            ...
            // Get USDAI from caller
            vat.move(msg.sender, vow, owe);

            // Removes USDai out for liquidation from accumulator
