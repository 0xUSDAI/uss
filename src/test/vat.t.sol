...
contract JoinTest is DSTest {
    ...
    function setUp() public {
        ...
        usdaiA = new USDaiJoin(address(vat), address(usdai));
        vat.rely(address(usdaiA));
        usdai.setOwner(address(usdaiA));
    ...
    function try_exit_usdai(address usr, uint wad) public returns (bool ok) {
        string memory sig = "exit(address,uint256)";
        (ok,) = address(usdaiA).call(abi.encodeWithSignature(sig, usr, wad));
    }
    ...
    function test_usdai_exit() public {
        address urn = address(this);
        vat.mint(address(this), 100 ether);
        vat.hope(address(usdaiA));
        assertTrue( try_exit_usdai(urn, 40 ether));
        assertEq(usdai.balanceOf(address(this)), 40 ether);
        assertEq(vat.usdai(me),              rad(60 ether));
        assertTrue( try_cage(address(usdaiA)));
        assertTrue(!try_exit_usdai(urn, 40 ether));
        assertEq(usdai.balanceOf(address(this)), 40 ether);
        assertEq(vat.usdai(me),              rad(60 ether));
    }
    function test_usdai_exit_join() public {
        address urn = address(this);
        vat.mint(address(this), 100 ether);
        vat.hope(address(usdaiA));
        usdaiA.exit(urn, 60 ether);
        usdai.approve(address(usdaiA), uint(-1));
        usdaiA.join(urn, 30 ether);
        assertEq(usdai.balanceOf(address(this)),     30 ether);
        assertEq(vat.usdai(me),                  rad(70 ether));
    }
    function test_cage_no_access() public {
        gemA.deny(address(this));
        assertTrue(!try_cage(address(gemA)));
        usdaiA.deny(address(this));
        assertTrue(!try_cage(address(usdaiA)));
    }
}
...
contract BiteTest is DSTest {
    ...
    function test_happy_bite() public {
        ...
        assertEq(vat.usdai(address(vow)), rad(0 ether));
        vat.mint(address(this), 100 ether);  // magic up some usdai for bidding
        flip.tend(auction, 40 ether,   rad(1 ether));
        flip.tend(auction, 40 ether, rad(110 ether));

        assertEq(vat.usdai(address(this)),  rad(90 ether));
        assertEq(gem("gold", address(this)), 960 ether);
        ...
        flip.deal(auction);
        assertEq(cat.litter(), 0);
        assertEq(vat.usdai(address(vow)),  rad(110 ether));
    }
    ...
    function test_partial_litterbox() public {
        ...
        assertEq(vat.usdai(address(this)),  rad(150 ether));
        assertEq(vat.usdai(address(vow)),     rad(0 ether));
        flip.tend(auction, 50 ether, rad(1 ether));
        assertEq(cat.litter(), rad(75 ether));
        assertEq(vat.usdai(address(this)), rad(149 ether));
        flip.tend(auction, 50 ether, rad(75 ether));
        assertEq(vat.usdai(address(this)), rad(75 ether));
        ...
        assertEq(vat.usdai(address(this)),   rad(75 ether));
        assertEq(vat.usdai(address(vow)),    rad(75 ether));
    }
    ...
    function test_partial_litterbox_realistic_values() public {
        ...
        uint vowBalance = vat.usdai(address(vow)); // Balance updates after vat.fold is called from jug
        ...
        assertEq(vat.usdai(address(this)), rad(150 ether));
        assertEq(vat.usdai(address(vow)),  vowBalance);
        flip.tend(auction, dink, rad( 1 ether));
        assertEq(cat.litter(), tab);
        assertEq(vat.usdai(address(this)), rad(149 ether));
        flip.tend(auction, dink, tab);
        assertEq(vat.usdai(address(this)), rad(150 ether) - tab);
        ...
        assertEq(vat.usdai(address(this)), rad(150 ether) - tab);  
        assertEq(vat.usdai(address(vow)),  vowBalance + tab);
    }
    ...
    function test_partial_litterbox_multiple_bites() public {
        ...
        flip.tend(auction, 50 ether, rad( 1 ether));
        assertEq(cat.litter(), rad(75 ether));
        assertEq(vat.usdai(address(this)), rad(74 ether));
        flip.tend(auction, 50 ether, rad(75 ether));
        ...
        assertEq(gem("gold", address(this)),  1000 ether);
        assertEq(vat.usdai(address(this)), 0);
        assertEq(vat.usdai(address(vow)),  rad(150 ether));
    }
    ...
    function test_flappy_bite() public {
        ...
        assertEq(vat.usdai(address(vow)),    rad(100 ether));
        assertEq(gov.balanceOf(address(this)), 100 ether);
        ...
        assertEq(vat.usdai(address(this)),     rad(0 ether));
        assertEq(gov.balanceOf(address(this)), 100 ether);
        flap.tend(id, rad(100 ether), 10 ether);
        hevm.warp(now + 4 hours);
        gov.setOwner(address(flap));
        flap.deal(id);
        assertEq(vat.usdai(address(this)),     rad(100 ether));
...
contract FoldTest is DSTest {
    ...
    function draw(bytes32 ilk, uint usdai) internal {
        vat.file("Line", rad(usdai));
        vat.file(ilk, "line", rad(usdai));
        vat.file(ilk, "spot", 10 ** 27 * 10000 ether);
        address self = address(this);
        vat.slip(ilk, self,  10 ** 27 * 1 ether);
        vat.frob(ilk, self, self, self, int(1 ether), int(usdai));
    }
    function test_fold() public {
        ...
        assertEq(vat.usdai(ali),      rad(0.05 ether));
    }
}
