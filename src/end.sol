...
// Copyright (C) 2020-2021 USDai Foundation
//
...
interface VatLike {
    function usdai(address) external view returns (uint256);
...
/*
    ...
    the final usdai / collateral price. In particular, we need to determine

      a. `gap`, the collateral shortfall per collateral type by
         considering under-collateralised CDPs.

      b. `debt`, the outstanding usdai supply after including system
         surplus / deficit
    ...
    We determine (b) by processing ongoing usdai generating processes,
    i.e. auctions. We need to ensure that auctions will not generate any
    further usdai income.
    ...
    4a. i) `wait`: set the cooldown period to be at least as long as the
           ...
           auction dynamics due to the now varying price of usdai.

       ii) `skip`: cancel all ongoing auctions and seize the collateral.

           This allows for faster processing at the expense of more
           processing calls. This option allows usdai holders to retrieve
    ...
    So the way to ensure the protocol will not receive extra usdai income is:
    ...
    6. `thaw()`:
       ...
       - fixes the total outstanding supply of usdai
    ...
    type and usdai holders can now turn their usdai into collateral. Each
    unit usdai can claim a fixed basket of collateral.

    USDai holders must first `pack` some usdai into a `bag`. Once packed,
    usdai cannot be unpacked and is not transferrable. More usdai can be
    added to a bag later.

    8. `pack(wad)`:
        - put some usdai into a bag in preparation for `cash`

    Finally, collateral can be obtained with `cash`. The bigger the bag,
    the more collateral can be released.

    9. `cash(ilk, wad)`:
        - exchange some usdai from your bag for gems from a specific ilk
...
contract End {
    ...
    uint256  public debt;  // Total outstanding usdai following processing [rad]
    ...
    function thaw() external {
        ...
        require(vat.usdai(address(vow)) == 0, "End/surplus-not-zero");
