...
contract Vatish is DSToken('') {
    ...
    function usdai(address usr) public view returns (uint) {
...
contract FlopTest is DSTest {
    ...
    function test_dent() public {
        ...
        assertEq(vat.usdai(ali), 190 ether);
        // gal receives payment
        assertEq(vat.usdai(gal),  10 ether);
        assertEq(Gal(gal).Ash(), 0 ether);

        Guy(bob).dent(id, 80 ether, 10 ether);
        // bid taken from bidder
        assertEq(vat.usdai(bob), 190 ether);
        // prev bidder refunded
        assertEq(vat.usdai(ali), 200 ether);
    ...
    function test_dent_Ash_less_than_bid() public {
        uint id = Gal(gal).kick(flop, /*lot*/ 200 ether, /*bid*/ 10 ether);
        assertEq(vat.usdai(gal),  0 ether);
    ...
    function test_yank() public {
        // yanking the auction should refund the last bidder's usdai, credit a
        ...
        assertEq(vat.usdai(ali), 200 ether);
        assertEq(vat.usdai(bob), 200 ether);
        assertEq(vat.usdai(gal), 0);
        ...
        assertEq(vat.usdai(ali), 200 ether);  // ali's usdai balance is unchanged
        assertEq(vat.usdai(bob), 190 ether);
        assertEq(vat.usdai(gal),  10 ether);
        ...
        // confirm final state
        assertEq(vat.usdai(ali), 200 ether);
        assertEq(vat.usdai(bob), 200 ether);  // bob's bid has been refunded
        assertEq(vat.usdai(gal),  10 ether);
    ...
    function test_yank_no_bids() public {
        ...
        // amounts of usdai (credited to the gal) and sin (credited to the caller of cage)
        // in practice, gal == (caller of cage) == (vow address)
        uint id = Gal(gal).kick(flop, /*lot*/ 200 ether, /*bid*/ 10 ether);
